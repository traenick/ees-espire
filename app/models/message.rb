...
class Message < ActiveRecord::Base
  ...
  def json_string
    self.to_json({:methods => [:date_resolved_string, :date_originated_string, :dollar_amt_float], :only => [:id, :body, :subject, :created_at],:include => [{:user => {:methods => :full_name, :only => [:first_name, :last_name]}}, {:message_links => {:except => [:id, :created_at, :updated_at]}}, {:message_flows => {:methods => :message_flow_status, :except => [:id, :updated_at, :message_id, :response_notes, :response_choice_id, :user_id], :include => {:user => {:methods => :full_name, :only => [:first_name, :last_name]}} }}, {:message_sub_type => {:methods => [:type_and_sub_type_name, :message_type_name], :only => [:sub_type_name], :include => {:response_choices => {:except => [:id, :created_at, :updated_at]}}}}]})
  end
